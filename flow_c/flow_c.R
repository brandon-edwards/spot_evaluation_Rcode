#rm(list=ls())
setwd("C:/Users/galujanm/Documents/ML Performance metrics/flow_c")
#host, score, rank
library(pROC)
library(ggplot2)
...
f5   <- list.files(pattern="flow_c_5_")
f10  <- list.files(pattern="flow_c_10_")
f15  <- list.files(pattern="flow_c_15_")
f20  <- list.files(pattern="flow_c_20_")
f30  <- list.files(pattern="flow_c_30_")
f50  <- list.files(pattern="flow_c_50_")
f100 <- list.files(pattern="flow_c_100_")
...
for (k in 1:length(topics)){
  ...
  for(j in 1:replicates){
    ...
    colnames(df) <- c('time','sip','dip','sport','dport','score','rank','original','type')
    ...
    df$TP <- -99
    df$FP <- -99
    df <- insertRow(df,rep(1,ncol(df)),1)
    df$rank <- as.numeric(df$rank)
    df <- df[order(df$rank),]
    df <- rbind(df,rep(1,ncol(df)))
    df$rank <- as.numeric(df$rank)
    ...
    for (i in 2:(nrow(df)-1)){
      ...
      df$FP[i] <- (df$rank[i]-1-i)/ totalRows
    }
    AUC <- 0
  ...
  ggplot(toplot,aes(x=rank,y=y)) + geom_point(pch = 21,position = position_jitter(width = 1)) +
    ylim(-0.5,.5) + xlim(-totalRows*.1,totalRows) +
